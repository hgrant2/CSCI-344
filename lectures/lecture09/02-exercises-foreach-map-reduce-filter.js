...
function tweetToImgURL(tweet) {
    return tweet.image_url;
}

function tweetsToImgUrls() {
    return statuses.map(
        tweet => tweet.image_url
        ).filter(image_url =>  image_url !== undefined);

//returns a list of strings (image Urls)
}

// const images = tweetsToImgUrls;
// console.log(images);
...
//     return statuses.filter(retweet_count => retweet_count >== 500).map(
...
const whichIsMostPopular = (tweetA, tweetB) => {
    return (tweetA.retweet_count > tweetB.retweet_count) ? tweetA : tweetB;
}

function getMostPopularTweet(){
    return statuses.reduce(whichIsMostPopular);
}

console.log(getMostPopularTweet());